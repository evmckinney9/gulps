...
use crate::gate_primitives::{kron_2x2, project_su4};
use crate::{Mat2, Mat4, C0, C1, C64, CI, CM1};

const WEYL_MATCH_TOL: f64 = 2e-5;
const TWO_PI: f64 = 2.0 * PI;
const THREE_HALVES_PI: f64 = 3.0 * FRAC_PI_2;

// Qiskit's B_NON_NORMALIZED (|entries| = 1, B†B = 2I).
// NOT gate_primitives::magic() (that one is B/√2 with a column permutation).
fn b_nonnorm() -> Mat4 {
    Mat4::new(
...
#[rustfmt::skip] // one-transform-per-line layout mirrors the ported algorithm
fn decompose(u: &Mat4) -> Result<KakFactors, String> {
    ...
    if cs[0] > FRAC_PI_2 {
        cs[0] -= THREE_HALVES_PI; k1l = k1l * ipy_m; k1r = k1r * ipy_m; global_phase += FRAC_PI_2;
    }
    if cs[1] > FRAC_PI_2 {
        cs[1] -= THREE_HALVES_PI; k1l = k1l * ipx_m; k1r = k1r * ipx_m; global_phase += FRAC_PI_2;
    }
    let mut conjs = 0;
    if cs[0] > FRAC_PI_4 {
        cs[0] = FRAC_PI_2 - cs[0];
    ...
    if cs[2] > FRAC_PI_2 {
        cs[2] -= THREE_HALVES_PI; k1l = k1l * ipz_m; k1r = k1r * ipz_m;
...
/// Match a decomposed target `t` against a decomposed basis `b`.
///
/// Tries a direct Weyl match, then a rho-reflected match, then a Frobenius
/// fallback that picks whichever of the two corrections reconstructs
/// `u_target` better. `ctx` names the caller for the error message.
fn match_recovery(
    t: &KakFactors,
    b: &KakFactors,
    u_target: &Mat4,
    ...
    // Frobenius fallback: try both, pick lowest reconstruction error
    ...
    Err(format!(
        "{ctx}: Weyl diffs ({:.2e}, {:.2e}, {:.2e}), best_err={:.2e}",
        (t.a - b.a).abs(),
        (t.b - b.b).abs(),
        (t.c - b.c).abs(),
        best_err,
...
/// Recover the local corrections relating u_basis to u_target.
pub fn recover_local_equivalence(
    ...
    match_recovery(&t, &b, u_target, u_basis, "recover")
}

/// Recovery for a canonical-gate target: its K-factors are identity, so
/// only the basis is decomposed.
fn recover_canonical_target(
    ...
    let b = decompose_basis(u_basis)?;
    let t_id = KakFactors {
        a: target_weyl[0] * FRAC_PI_2,
        b: target_weyl[1] * FRAC_PI_2,
        c: target_weyl[2] * FRAC_PI_2,
        global_phase: 0.0,
        k1l: Mat2::identity(),
        k1r: Mat2::identity(),
        k2l: Mat2::identity(),
    ...
    let u_target =
        crate::gate_primitives::canonical_matrix(target_weyl[0], target_weyl[1], target_weyl[2]);
    match_recovery(&t_id, &b, &u_target, u_basis, "recover_canonical")
}
