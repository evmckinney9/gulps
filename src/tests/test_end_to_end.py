...
def test_local_unitary():
    ...
    assert_fidelity(target, GulpsDecomposer(isa=isa)(target), "local")
...
def test_exact_isa_gate():
    ...
    assert_fidelity(gate, GulpsDecomposer(isa=isa)(gate), "exact ISA gate")
...
def test_random_unitaries(decomposer):
    ...
    for seed in range(N_RANDOM):
        ...
        assert_fidelity(u, decomposer(u), f"seed={seed}")
...
def test_random_unitaries_no_precompute(decomposer_no_precompute):
    ...
    for seed in range(N_RANDOM):
        u = random_unitary(4, seed=seed)
        assert_fidelity(u, decomposer_no_precompute(u), f"seed={seed}")
