...
from hetero_isas.sqrtiswap.siswap import SiSwapGate
from hetero_isas.sqrtiswap.utils import average_infidelity, find_min_point
...
class SiSwapDecomposer:
    """A class for decomposing 2-qubit unitaries into at most 3 uses of the
    sqrt(iSWAP) gate.
    ...
    def __call__(
        ...
        *,
        ...
        phase_diff = cmath.phase(
            Operator(unitary).data.flat[i] / Operator(circuit).data.flat[i]
        )
...
def _can_circuit(x, y, z):
    """Find a circuit that implements the canonical gate CAN(x, y, z) := RXX(x)
    .
        ...
        if z < 0:  # blue region
            ...
            for instruction in inverse_circuit:
                if isinstance(instruction.operation, SiSwapGate):
                    ...
                    inverse_circuit_with_siswap_dg.append(
                        SiSwapGate().inverse(), [0, 1]
                    )
                    inverse_circuit_with_siswap_dg.z(0)
                else:
            ...
            if x > np.pi / 8:  # green region
                ...
                if (
                    y + z < np.pi / 4
                ):  # eigenphase crossing condition: a_2 - pi/4 < a_3 + pi/4
...
def _interleaving_single_qubit_gates(x, y, z):
    """Find the single-qubit gates given the interaction coefficients (x, y, z)
    ∈ W′ when sandwiched by two SiSwap gates.
