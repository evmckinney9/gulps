"""Qiskit unitary-synthesis plugin entry point for GULPS."""

from __future__ import annotations

import numpy as np
from qiskit.dagcircuit import DAGCircuit
from qiskit.transpiler.passes.synthesis.plugin import UnitarySynthesisPlugin

from gulps.gulps_decomposer import GulpsDecomposer


class GulpsSynthesisPlugin(UnitarySynthesisPlugin):
    """Qiskit synthesis plugin for two-qubit unitaries using GULPS."""

    _decomposer = None

    @property
    def min_qubits(self):
        """Minimum number of qubits this plugin handles."""
        return 2

    @property
    def max_qubits(self):
        """Maximum number of qubits this plugin handles."""
        return 2

    @property
    def supports_target(self):
        """Whether the plugin uses the Target object."""
        return True

    ######################################
    # NOTE, I avoid using basis_gates and gate_errors to focus on implementation
    # that uses the target object
    @property
    def supports_basis_gates(self):
        """Whether the plugin uses basis_gates."""
        return False

    @property
    def supports_gate_errors(self):
        """Whether the plugin uses gate error rates."""
        return False

    # NOTE, using this one seems better than supports_gate_errors
    # but leave False for now
    @property
    def supports_gate_errors_by_qubit(self):
        """Whether the plugin uses per-qubit gate errors."""
        return False

    @property
    def supports_coupling_map(self):
        """Whether the plugin uses a coupling map."""
        return False

    @property
    def supports_natural_direction(self):
        """Whether the plugin uses natural gate direction."""
        return False

    @property
    def supports_pulse_optimize(self):
        """Whether the plugin supports pulse-level optimisation."""
        return False

    @property
    def supports_gate_lengths(self):
        """Whether the plugin uses gate duration data."""
        return False

    @property
    def supports_gate_lengths_by_qubit(self):
        """Whether the plugin uses per-qubit gate durations."""
        return False

    @property
    def supported_bases(self):
        """Return supported basis gate sets (None = any)."""
        return None

    def run(self, unitary: np.ndarray, **options) -> DAGCircuit:
        """Synthesise a two-qubit unitary using GULPS."""
        # basis_gates = options.get("basis_gates", None)
        target = options.get("target")
        if target is None:
            raise ValueError("Target must be provided in options.")

        # parse target into gate_set list and cost list
        gate_set = []
        costs = []
        for idx, instruction in enumerate(target.operations_for_qargs((0, 1))):
            error = target.instruction_properties(idx).error
            gate_set.append(instruction)
            costs.append(error)

        if (
            GulpsSynthesisPlugin._decomposer is None
            or GulpsSynthesisPlugin._decomposer.isa.gate_set != gate_set
        ):
            GulpsSynthesisPlugin._decomposer = GulpsDecomposer(
                gate_set=gate_set,
                costs=costs,
                precompute_polytopes=None,
            )

        return GulpsSynthesisPlugin._decomposer(target=unitary, return_dag=True)
