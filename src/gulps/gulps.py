...
class SolovayKitaevDecomposition:
    ...
    def __init__(
        ...
        basic_approximations: (
            str | dict[str, np.ndarray] | list[GateSequence] | None
        ) = None,
        *,
        ...
        if basic_approximations is None:
            ...
            if (
                isinstance(basic_approximations, str)
                and basic_approximations[~3:] != ".npy"
            ):
    ...
    def run(
        ...
        if return_dag:
            from qiskit.converters import (
                circuit_to_dag,  # pylint: disable=cyclic-import
            )
