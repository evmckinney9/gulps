...
def find_min_point(P):
    """Find the closest point on convex polytope P to the origin (in Euclidean
    distance).
    ...
    while True:
        ...
        # Step 4.
        P_aux = [
            p for p in P_Pk if (np.dot(y - x, y - p) > _EPS) and (np.dot(x, y - p) != 0)
        ]
        p_lambda = P_aux[
            np.array([np.dot(y, y - p) / np.dot(x, y - p) for p in P_aux]).argmin()
        ]
...
def average_infidelity(p, q):
    ...
    Average gate fidelity is :math:`Fbar = (d + |Tr (Utarget \\cdot
    U^dag)|^2) / d(d+1)`
